...
def extract_1099_int(source: OcrResult | str) -> dict[str, Any]:
    ...
    # Interest income: labeled box — prefer Form 1099-INT Copy B amount over summary totals
    m_copy_b = re.search(r"(?:Copy\s*B|F[oa]?r?m\s*1099-INT)[\s\S]{0,300}?\$\s*([0-9,]+\.\d{2})", eng.text, re.I)
    if m_copy_b:
        fields["1 Interest Income"] = normalize_money(m_copy_b.group(1))


    elif not fields.get("1 Interest Income"):
        from app.ocr.field_engine import normalize_money
