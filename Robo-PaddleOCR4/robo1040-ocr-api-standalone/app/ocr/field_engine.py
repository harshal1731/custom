...
def _group_lines(tokens: list[OcrToken], y_tol: int = 18, max_gap: int = 40) -> list[str]:
    if not tokens:
        return []
...
class SpatialFieldEngine:
    ...
    def from_ocr(ocr: OcrResult, prefer_left_half: bool = False) -> SpatialFieldEngine:
        ...
        page = ocr.pages[0]
        if prefer_left_half and page.width > 0 and page.tokens:
            mid = page.width / 2
            has_left = any(t.cx < mid * 0.9 for t in page.tokens)
            has_right = any(t.cx > mid * 1.1 for t in page.tokens)
            if has_left and has_right:
                toks = [t for t in page.tokens if t.cx <= page.width * 0.52]
    ...
    def _resolve(self, spec: FieldSpec, parties: dict[str, dict[str, str | None]]) -> Any:
        ...
        if vt == "address_block":
            ...
            if part == "combined":
                if "employee" in (spec.key or "").lower() or spec.role == "employee":
                    bits = [party.get("street"), party.get("city")]
                else:
        ...
        if vt == "name_split":
            ...
            if len(parts) >= 4 and len(parts) % 2 == 0:
                ...
                if [p.lower() for p in parts[:half]] == [p.lower() for p in parts[half:]]:
                    parts = parts[:half]
            if spec.part == "first":
                return " ".join(parts[:-1]) if len(parts) >= 2 else (parts[0] if parts else None)
            if spec.part == "last":
    ...
    def _party_after_labels(self, labels: list[str]) -> dict[str, str | None]:
        ...
        for j in range(i + 1, min(i + 10, len(self.lines))):
            ln_text = self.lines[j].text
            if re.search(r"^(?:d\s*Control|e\s*Employee|c\s*Employer|7\s*Social|15\s*State|Form\s*W-2|Copy\s*[ABC])", ln_text, re.I):
                break
            chunk.append(ln_text)
    ...
    def _parse_address_triplet(self, lines: list[str]) -> dict[str, str | None]:
        ...
        for ln in useful:
            sm = _STREET_RE.search(ln)
            cm = self._city_match(ln)
            if sm and cm and "," in ln:
                idx = sm.start()
                c_idx = ln.rfind(",") if "," in ln else len(ln)
                n_part = clean(ln[:idx].rstrip(" ,"))
                s_part = clean(ln[idx:c_idx].rstrip(" ,"))
                c_part = clean(ln[c_idx:].lstrip(" ,"))
                if n_part and s_part and c_part:
                    return {
                        "name": self._scrub_name(n_part),
                        "street": self._norm_street(s_part),
                        "city": self._norm_city(c_part),
                    }
        ...
        for ln in useful:
            ...
            if name and not street and _STREET_RE.search(ln):
                sm = _STREET_RE.search(ln)
                street = self._norm_street(sm.group(0))
                rest = ln[sm.end():].strip(" ,")
                if rest and self._city_match(rest):
                    city = self._norm_city(self._city_match(rest) or rest)
                continue
