...
def _group_lines(tokens: list[OcrToken], y_tol: int = 4) -> list[str]:
    if not tokens:
        return []
...
def extract_digital_pdf(pdf_bytes: bytes, max_pages: int = 1) -> OcrResult:
    ...
    for idx in range(limit):
        ...
        # Extract text by blocks to prevent multi-column layouts (like side-by-side addresses)
        # from being interlaced line-by-line. This keeps name and address groups intact.
        ...
        for b in blocks:
            ...
            if block_text:
                # Add each line in the block, then a blank line to separate blocks
        ...
        # If spaced-character collapsing was used, rebuild lines from the merged tokens 
        # so they match what the rest of the pipeline expects
        if needs_collapse:
